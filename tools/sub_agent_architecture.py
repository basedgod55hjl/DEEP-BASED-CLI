...
class CoordinatorAgent(BaseAgent):
    ...
    def __init__(self, agent_id: str, sub_agents: List[BaseAgent]):
        super().__init__(agent_id, AgentType.COORDINATOR, ["task_delegation", "workflow_management"])
    ...
    async def execute_task(self, task: Task) -> TaskResult:
        ...
        try:
            ...
            if not suitable_agents:
                ...
                if not suitable_agents:
                    raise Exception(f"No suitable agent found for task type: {task.agent_type}")
    ...
    async def delegate_complex_task(self, description: str, subtasks: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    def get_system_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "sub_agents": {agent_id: agent.get_status() for agent_id, agent in self.sub_agents.items()},
...
class CoderAgent(BaseAgent):
    ...
    def __init__(self, agent_id: str, llm_client):
        super().__init__(agent_id, AgentType.CODER, ["code_generation", "code_review", "refactoring"])
...
class AnalyzerAgent(BaseAgent):
    ...
    def __init__(self, agent_id: str, llm_client):
        super().__init__(agent_id, AgentType.ANALYZER, ["code_analysis", "performance_analysis", "security_analysis"])
        self.llm_client = llm_client
...
class ResearcherAgent(BaseAgent):
    ...
    def __init__(self, agent_id: str, llm_client, search_tool):
        super().__init__(agent_id, AgentType.RESEARCHER, ["web_search", "documentation_search", "research_synthesis"])
        self.llm_client = llm_client
...
class AgentFactory:
    ...
    def create_agent(agent_type: AgentType, agent_id: str, **kwargs) -> BaseAgent:
        ...
        if agent_type == AgentType.CODER:
            ...
            return ResearcherAgent(agent_id, kwargs.get('llm_client'), kwargs.get('search_tool'))
        else:
...
class SubAgentSystem:
    ...
    def __init__(self, llm_client, search_tool=None):
        ...
        # Create sub-agents
        self.agents = {
            "coder": AgentFactory.create_agent(AgentType.CODER, "coder_001", llm_client=llm_client),
            "analyzer": AgentFactory.create_agent(AgentType.ANALYZER, "analyzer_001", llm_client=llm_client),
            "researcher": AgentFactory.create_agent(AgentType.RESEARCHER, "researcher_001", llm_client=llm_client, search_tool=search_tool)
        }
        
        # Create coordinator
        self.coordinator = CoordinatorAgent("coordinator_001", list(self.agents.values()))
    
    async def execute_complex_task(self, description: str, subtasks: List[Dict[str, Any]]) -> Dict[str, Any]:
        """Execute a complex task using sub-agents"""
        return await self.coordinator.delegate_complex_task(description, subtasks)
    
    async def generate_code_with_analysis(self, requirements: str, language: str = "python") -> Dict[str, Any]:
...
async def test_sub_agent_system():
    ...
    class MockSearchTool:
        async def search(self, query: str) -> Dict[str, Any]:
            ...
            return {"results": [{"title": f"Result for {query}", "snippet": "Sample result"}]}
