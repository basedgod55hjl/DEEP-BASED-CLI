...
// Default API key
const DEFAULT_API_KEY = 'sk-ca68d65584b14a7f998db0a1a0548d5c';
...
export class ConfigManager {
  ...
  private loadConfig(): DeepSeekConfig {
    ...
    const envConfig: DeepSeekConfig = {
      apiKey: process.env.DEEPSEEK_API_KEY || DEFAULT_API_KEY,
    ...
    if (existsSync(this.configFile)) {
      try {
        ...
        // Merge configs, with env taking precedence, then file, then default
        return { 
          apiKey: envConfig.apiKey || fileConfig.apiKey || DEFAULT_API_KEY,
          apiEndpoint: envConfig.apiEndpoint || fileConfig.apiEndpoint,
          model: envConfig.model || fileConfig.model,
          maxTokens: envConfig.maxTokens || fileConfig.maxTokens,
          temperature: envConfig.temperature || fileConfig.temperature,
        };
      } catch (error) {
